const input = ["2", "1 1 1", "2 0 4", "3", "1 0 0", "0 1 0", "0 0 0", "0"];
...
while (true) {
  ...
  for (let i = 0; i < n; i++) {
    [cards[0][i], cards[1][i], cards[2][i]] = gets()
...
function canWinWith(a, b, c) {
  ...
  if (
    a < n &&
    b < n &&
    (cards[0][a] + cards[1][b]) % 3 == 0 &&
    canWinWith(a + 1, b + 1, c)
  ) {
    m[s] = 1;
    return true;
  }
  ...
  if (
    a < n &&
    c < n &&
    (cards[0][a] + cards[2][c]) % 3 == 0 &&
    canWinWith(a + 1, b, c + 1)
  ) {
    m.set(s, 1);
    return true;
  }
  if (
    b < n &&
    c < n &&
    (cards[1][b] + cards[2][c]) % 3 == 0 &&
    canWinWith(a, b + 1, c + 1)
  ) {
    m.set(s, 1);
    return true;
  }
  ...
  if (
    ...
    b < n &&
    c < n &&
    (cards[0][a] + cards[1][b] + cards[2][c]) % 3 == 0 &&
    canWinWith(a + 1, b + 1, c + 1)
  ) {
    m.set(s, 1);
    return true;
  }
