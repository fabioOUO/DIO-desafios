const entrada = ["abcdef abc abc abc"];

let position = 0;
const gets = () => entrada[position++];
...
while (true) {
  ...
  const words = uniqueWords
    .map((word) => {
      const repeatations = sentence
        .join(" ")
        .match(new RegExp(`\\b${word}\\b`, "g")).length;
  ...
  const abbreviations = alphabet
    .split("")
    .map((letter) => {
      candidates = words.filter((word) =>
        word.word.match(new RegExp(`\\b${letter}\\w{2,}\\b`, "g"))
      );
      ...
      return candidates.reduce(
        (acc, curr) => {
          return curr.save >= acc.save ? curr : acc;
        },
        { save: 0 }
      );
    })
